...
describe('Order to takeover synchronization MailShip-MailWise test', () => { //[MW-1582]
    before(() => {
    ...
    it('Should return a dispatch in status ready to takeover in MailWise', {
        retries
    }, () => {
        skipOn(isDev());
        const variables = {
        ...
        cy.request({
            ...
            expect(dispatch.status).to.eq('READY_TO_TAKEOVER');
            customerId = dispatch.order.customerId;
        });
    });

    it('Should return an order in status ready to takeover in MailWise', {
        retries
    }, () => {
        skipOn(isDev());
        cy.wait(5000);
        ...
        cy.request({
            ...
            expect(node.status).to.eq('READY_TO_TAKEOVER');
        });
    });

    it('Should return an expedition in status ready to takeover in MailShip', {
        retries
    }, () => {
        skipOn(isDev());
        cy.wait(10000);
        cy.request({
            ...
            expect(expedition.status).to.eq('ready_to_takeover');
        });
    });
});
