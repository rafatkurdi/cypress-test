...
describe('Order returning synchronization MailShip-MailWise E2E test', () => { //[MW-1443, MW-1471, MW-1486, MW-1557]
    before(() => {
    ...
    it('Should return a workstation in MailWrap', () => {
        cy.request({
            ...
            const result = workstations.edges[0].node;
    ...
    it('Should return a dispatch in status waiting for the carrier in MailWise', {
        retries
    }, () => {
        skipOn(isDev());
        const variables = {
    ...
    it('Should return an order in status returning in MailWise', {
        ...
        cy.request({
            ...
            const organisationStore = node.organisationStore;
            expect(organisationStore).to.have.all.keys(PropertiesMW.organisationStore);
            orderId = node.id;
            organisationStoreId = organisationStore.id;
        });
    });
    ...
    it('Should create a return receipt in MailWise', {
        retries
    }, () => {
        skipOn(isDev());
        const variables = {
        ...
        cy.request({
            ...
            url: urlMailWise,
            headers: mwHeaders,
            body: { query: mutations.returnReceiptCreate, variables },
            failOnStatusCode: false
        }).then(response => {
            expect(response).to.have.property('status', 200);
            const returnReceipt = response.body.data.returnReceiptCreate.receipt;
            expect(returnReceipt).to.have.all.keys(PropertiesMW.returnReceipt);
            expect(returnReceipt.type).to.eq('RETURN');
            returnReceiptId = returnReceipt.id;
        });
    });

    it('Should add item from receipt to location in MailWise', {
        retries
    }, () => {
        skipOn(isDev());
        const variables = {
        ...
        cy.request({
            ...
            url: urlMailWise,
            headers: mwHeaders,
            body: { query: mutations.returnReceiptAddItemLocationDraft, variables },
            failOnStatusCode: false
        }).then(response => {
            expect(response).to.have.property('status', 200);
            const returnReceiptAddItemLocation = response.body.data.returnReceiptAddItemLocationDraft.receiptItemLocationDraft;
            expect(returnReceiptAddItemLocation).to.have.all.keys(PropertiesMW.receiptItemLocationDraft);
            expect(returnReceiptAddItemLocation.number).to.eq(variables.number);
        });
    });
    ...
    it('Should finished the receipt in MailWise', {
        retries
    }, () => {
        skipOn(isDev());
        const variables = {
        ...
        cy.request({
            ...
            url: urlMailWise,
            headers: mwHeaders,
            body: { query: mutations.receiptForceFinish, variables: variables },
            failOnStatusCode: false
        }).then(response => {
            expect(response).to.have.property('status', 200);
            const receipt = response.body.data.receiptForceFinish;
            const result = receipt.receipt;
            expect(result).to.have.key('status');
            expect(result.status).to.eq('FINISHED');
            returnReceiptId = receipt.id;
        });
    });

    it('Should return a dispatch in status returned in MailWise', {
        retries
    }, () => {
        skipOn(isDev());
        const variables = {
        ...
        cy.request({
            ...
            expect(dispatch).to.have.all.keys(PropertiesMW.dispatchWrap);
            expect(dispatch.id).to.eq(variables.id);
            expect(dispatch.status).to.eq('RETURNED');
            const orderItem = dispatch.items[0].orderItem;
            const product = orderItem.product;
            expect(product.id).to.eq(responseProduct.id);
            expect(product.productSku).to.eq(responseProduct.productSku);
    ...
    it('Should return an order in status returned in MailWise', {
        retries
    }, () => {
        skipOn(isDev());
        cy.wait(5000);
        ...
        cy.request({
            ...
            expect(node).to.have.all.keys(PropertiesMW.order);
            expect(node.customerId).to.eq(responseExpedition.orderNumber);
            expect(node.status).to.eq('RETURNED');
        });
    });

    it('Should return an expedition in status returned up in MailShip', {
        retries
    }, () => {
        skipOn(isDev());
        cy.wait(10000);
        cy.request({
            ...
            expect(response).to.have.property('status', 200);
            expect(response.body.id).to.eq(expeditionId);
            expect(response.body.status).to.eq('returned');
        });
    });
});
